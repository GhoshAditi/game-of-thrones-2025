...
export async function generateMockData(count: number): Promise<EventData[]> {
    ...
    const eventNames = ["Hackathon", "Coding Contest", "Design Challenge", "Quiz Competition"]
    const types = ["solo", "duo", "team"]
    const colleges = ["MIT", "Stanford", "Harvard", "Caltech", "Oxford"]
    const paymentStatuses: ("Verified" | "Not Verified")[] = ["Verified", "Not Verified"]
    ...
    return Array.from({ length: count }, (_, i) => {
        ...
        const teamMembers: TeamMember[] = Array.from({ length: teamSize }, (_, j) => ({
        ...
        return {
            ...
            registeredAt: new Date(Date.now() - Math.floor(Math.random() * 10000000000)).toLocaleString(),
            teamMembers,
        }
    })
}
