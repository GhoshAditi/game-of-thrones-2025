...
export const  generateEventRegistrations = (count: number): EventRegistration[] => {
    const events = [
        'Code Blast',
        'Robo War',
        'HackNova',
    ...
    const firstNames = ['Aarav', 'Anika', 'Vihaan', 'Sanya', 'Reyansh', 'Diya', 'Arjun', 'Myra', 'Advait', 'Prisha'];
    const lastNames = ['Sharma', 'Verma', 'Patel', 'Singh', 'Kumar', 'Gupta', 'Malhotra', 'Reddy', 'Choudhary', 'Mehta'];
    ...
    for (let i = 0; i < count; i++) {
        ...
        const registration: EventRegistration = {
            ...
            teamName: isTeam ?
                `${['Cyber', 'Tech', 'Data', 'Code', 'AI'][Math.floor(Math.random() * 5)]} ${['Warriors', 'Masters', 'Pioneers', 'Ninjas', 'Wizards'][Math.floor(Math.random() * 5)]
                } Team` : null,
            collegeName: colleges[Math.floor(Math.random() * colleges.length)],
            name: getRandomName(),
            teamLeadNumber: isTeam ? getRandomPhoneNumber() : null,
            transactionId: paymentVerified ? `TXN${Math.random().toString(36).substr(2, 12).toUpperCase()}` : null,
            members: [],
